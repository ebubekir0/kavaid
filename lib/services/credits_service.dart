...
class CreditsService extends ChangeNotifier {
  ...
  bool get isEntitlementPending => _purchaseManager.isEntitlementPending;

  /// Daha önce promo kullanıp kullanmadığı
  bool get hasUsedPromo => _purchaseManager.hasUsedPromo;
  ...
  Future<void> initialize() async {
    debugPrint(
      '🔄 [CreditsService] initialize() - PurchaseManager\'a yönlendirildi',
    );
    await _purchaseManager.refreshEntitlements();
  }
  ...
  Future<void> activatePremiumForever() async {
    debugPrint(
      '⚠️ [CreditsService] activatePremiumForever() çağrıldı - DEVRE DIŞI',
    );
  ...
  Future<void> activatePremiumMonthly() async {
    debugPrint(
      '⚠️ [CreditsService] activatePremiumMonthly() çağrıldı - DEVRE DIŞI',
    );
    // Artık Play Store üzerinden yapılmalı
  }
  ...
  Future<bool> togglePremiumStatus() async {
    debugPrint(
      '⚠️ [CreditsService] togglePremiumStatus() çağrıldı - DEVRE DIŞI',
    );
    return isPremium;
  }
  ...
  Future<void> setLifetimeAdsFree(bool value) async {
    debugPrint(
      '⚠️ [CreditsService] setLifetimeAdsFree() çağrıldı - DEVRE DIŞI',
    );
