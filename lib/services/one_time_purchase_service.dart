...
class OneTimePurchaseService extends ChangeNotifier {
  ...
  String get removeAdsPrice =>
      '₺69.99'; // Placeholder, offering'den çekilebilir.

  // LEGACY: Eski kodlarla uyumluluk için
  List<dynamic> get products =>
      []; // Artık kullanılmıyor ama hata vermemesi için
  bool get isAvailable => true;
  bool get purchasePending => false;
  bool get hasError => _lastError.isNotEmpty;

  Future<void> initialize() async {
    await PurchaseManager().bootstrapEntitlements();
  ...
  void _updateStatus(CustomerInfo info) {
    final bool newStatus =
        info.entitlements.all[_removeAdsEntitlement]?.isActive ?? false;
  ...
  Future<bool> buyRemoveAds() async {
    try {
      await PurchaseManager().initialize(fetchOfferings: false);
      _lastError = '';
      ...
      final lifetimePackage = offerings
          .current
          ?.lifetime; // RevenueCat dashboard'da "Lifetime" olarak işaretlenmeli

      if (lifetimePackage != null) {
        final info = await Purchases.purchasePackage(lifetimePackage);
        _updateStatus(info);
        if (_isLifetimeAdsFree) {
          await TurkceAnalyticsService.premiumSatinAlinaBasarili(
            'tek_seferlik',
            0,
